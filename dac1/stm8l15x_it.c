...
/**
  ...
  * @retval None
  */
static uint8_t shag=0;
//extern const uint16_t sin_tab[];
uint16_t tab(uint8_t sh);
//uint16_t tab2(uint8_t sh);
void period_inc(void);
INTERRUPT_HANDLER(TIM4_UPD_OVF_TRG_IRQHandler,25)
{
    
	//DAC_SetChannel1Data(DAC_Align_12b_R, s[shag++]);
	DAC_SetChannel1Data(DAC_Align_12b_R, tab(shag++));
	if (shag==200) shag=0;	
	DAC_SoftwareTriggerCmd(DAC_Channel_1, ENABLE);
	DAC_SoftwareTriggerCmd(DAC_Channel_1, DISABLE);
	period_inc();
		// Инвертируем выход пина 
